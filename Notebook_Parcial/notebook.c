...
//------------------------------------------------------------
void mostrarNotebook(eNotebook note, eTipo tipos[], int tamTipo, eMarca marcas[], int tamMarca, eCliente clientes[], int tamCliente, Prestamo prestamo[], int tamPrestamo){

        char descripcionMarca [20];
        char descripcionTipo[20];
        char nombreCliente[20];
        Prestamo auxprestamo;
        cargarPrestamoDatos(auxprestamo,prestamo,tamPrestamo,note.idPrestamo);
        ...
        printf("%d 	%10s   %10s   %10s    %10s   %.2f %.2f %d  %d\n", note.id, nombreCliente, note.modelo, descripcionMarca,
               descripcionTipo, note.precio, auxprestamo.importe,auxprestamo.cuotas,auxprestamo.saldado);


}

//-----------------------------------------------------------------
void mostrarNotebooks(eNotebook notebooks[], int tam, eTipo tipos[], int tamTipo, eMarca marcas[], int tamMarca, eCliente clientes[], int tamCliente,Prestamo prestamos[],int tamPrestamo){

 int flag=0;
 system("cls");
 printf("****** LISTA NOTEBOOKS ******\n");
 printf("ID	     CLIENTE    MODELO	        MARCA     TIPO	     PRECIO	 importePrestamo  cuotas  saldado\n");

    for(int i=0; i<tam; i++)
    {
        if(notebooks[i].isEmpty == 0)
        {
            mostrarNotebook(notebooks[i], tipos, tamTipo, marcas, tamMarca, clientes, tamCliente, prestamos,tamPrestamo);
            flag=1;
        }
